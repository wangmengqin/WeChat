<template>
	<div>
		...
		<div class="chatFooter">
			...
			<input type="text" v-show="isShowInput" id="mess"  v-model="sendData"/>
	...
	export default {
		data() {
			return {
				...
				chatphoto:'',
				insertId:'',
				socket:io("http://120.79.182.135:5555")
			}
		},
		methods: {
			checkShow() {
			...
			sendMessage(){
				...
				$.ajax({
					type:"post",
					url:"http://120.79.182.135:5555/saveChatInfo",
					async:true,
					dataType:'json',
					data:{
						message:_this.sendData,
...
//			var socket = io("http://120.79.182.135:5555");
//			socket.emit("addUser",{
...
//			})
			this.socket.on('returnMess',function(data){
				...
				$.ajax({
					type:"post",
					url:"http://120.79.182.135:5555/saveReturnMess",
					async:true,
					data:{
						id : _this.insertId,
						returnMess:data,
			...
			$.ajax({
				type:"post",
				url:"http://120.79.182.135:5555/getMyChat",
				async:true,
				dataType:'json',
				data:{
					username:_this.$store.state.name,
